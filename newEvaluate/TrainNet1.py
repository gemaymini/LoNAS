...
from newEvaluate.ConstructNet_dropout import ConstructNet
...
class TrainNet():
    ...
    def train(self,individual):
        ...
        with open(os.path.join(self.log_path,"test_acc.log"),"w") as test_f:
            with open(os.path.join(self.log_path,"train_acc.log"),"w") as train_f:
        ...
        self.log.info("re,no es :with best_acc:{} and val_acc".format(best_acc))
...
if __name__=="__main__":
    ...
    with open("group.txt","a") as f:
        for i in range(10):
            with open(os.path.join(path,indis[i]),"rb") as file:
