...
#nohup python train_imagenet.py  --arch fast10 --auxiliary --batch_size 256 --learning_rate 0.5 --gpu 2,3 &
...
parser.add_argument('--epochs', type=int, default=160, help='num of training epochs')
...
parser.add_argument('--tmp_data_dir', type=str, default='/home/cv_dataset/ImageNet', help='temp data dir')
...
def main():
    ...
    with open(os.path.join("../Output/4.23/v4.23/trained_indis","5010_4.41_95.51.txt"),"rb") as file:
        indi=pickle.load(file)
    model=ConstructNet(indi,CLASSES)
    print(model)
    if num_gpus > 1:
    ...
    #     valid_data, batch_size=args.batch_size, shuffle=False, pin_memory=False, num_workers=args.workers)
    data_loader=Evolve_DataLoader(1,False)
    train_queue,valid_queue=data_loader.data_loader()
